...
void loop()
{
	...
	float arrayrange[10][2];
	int x = 0;
	int y = 1;
	arrayrange[0][0] = 243.00;
	arrayrange[0][1] = -52.00;
	arrayrange[1][0] = 248.00;
	arrayrange[1][1] = -42.00;
	float Xmax = 0;
	float Xmin = 0;
	float Ymax = 0;
	float Ymin = 0;
	if (arrayrange[0][0] > arrayrange[1][0])
	{
		Xmax = arrayrange[0][0];
		Xmin = arrayrange[1][0];
	...
	if (arrayrange[0][y] > arrayrange[1][y])
	{
		Ymax = arrayrange[0][y];
		Ymin = arrayrange[1][y];
	...
	if (raw.XAxis <= Xmax && raw.XAxis >= Xmin) {
		Serial.println("x ok");

	}
	if (raw.YAxis <= Ymax && raw.YAxis >= Ymin) {
		Serial.println("Y  ok");
	}
	Serial.print(" Xraw = ");
