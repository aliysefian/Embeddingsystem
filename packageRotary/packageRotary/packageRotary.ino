...
void loop() {// call sensors.requestTemperatures() to issue a global temperature
	...
	if (valueSence != -127) {
		...
		long time = millis();
		while ((millis() - time < 10000) && flagClearScreen)
		{
...
*/
	...
	myGLCD.print(String(lowtemp, 1), 28, 2);
	myGLCD.setFont(BigNumbers);
	myGLCD.print(String(globalSensorValue, 1), 1, 20);
	myGLCD.setFont(SmallFont);
...
void PinA() {
	...
	if (reading == B00001100 && aFlag) { //check that we have both pins at detent (HIGH) and that we are expecting detent on this pin's rising edge
		...
		Serial.println("valllll-----:" + String(encoderPos))
